...
public class TelemetryLogger : MonoBehaviour
{
    ...
    public static void LogPlayerDeath(string causeOfDeath, Vector3 position, 
      string killer ="", Vector3 killerPosition = default) {
        TelemetryEvent data = default;
    ...
    public static void LogEnemyKill(string causeOfDeath, Vector3 position,
      string victim, Vector3 victimPosition) {
        TelemetryEvent data = default;
        data.eventType = "Kill";
        data.fromAttribute = causeOfDeath;
        data.fromPosition = position;
        data.toAttribute = victim;
        data.toPosition = victimPosition;
        data.numberAttribute = Counters.GetCurrentCount(Counters.CounterType.EnemyKill) + 1;
        _instance.LogTelemetry(data);
    }

    public static void LogCollect(string kind, Vector3 position, string name, float total = 0) {
        TelemetryEvent data = default;
        data.eventType = "Collect";
        data.fromAttribute = kind;
        data.fromPosition = position;
        data.toAttribute = name;
        data.numberAttribute = total;
        _instance.LogTelemetry(data);
    }

    public static void LogEnterZone(string zone, Vector3 position, int total = 0) {
        TelemetryEvent data = default;
        data.eventType = "EnterZone";
        data.fromAttribute = zone;
        data.fromPosition = position;
        data.numberAttribute = total;
        _instance.LogTelemetry(data);
    }

    public static void LogVictory(string victoryType, Vector3 position) {
        TelemetryEvent data = default;
        data.eventType = "Victory";
        data.fromAttribute = victoryType;
        data.fromPosition = position;
        data.numberAttribute = Counters.GetTime();
        _instance.LogTelemetry(data);
    }
